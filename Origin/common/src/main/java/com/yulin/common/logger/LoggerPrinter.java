...
final class LoggerPrinter implements Printer {
     ...
     *
     * @param tag is the given string which will be used in Logcat
     */
    @Override
     ...
     * 输出log内容到一行显示
     */
    private void logSingleLine(int priority, String tag, String message, Throwable throwable) {
        if (settings.getLogLevel() == LogLevel.NONE) {
            return;
        }
    ...
    private synchronized void log(int priority, Throwable throwable, String msg, Object... args) {
        ...
        // 如果是warning或error，使用多行输出，其它类型Log，使用单行输出。
        if (priority == ERROR || priority == WARN) {
            log(priority, tag, message, throwable);
        } else {
            logSingleLine(priority, tag, message, throwable);
        }
    }
